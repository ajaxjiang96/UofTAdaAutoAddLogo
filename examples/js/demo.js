...
$(document).ready(function() {
    ...
    let colors = {
        ...
        "greyWhite": ["Grey White", "#c1bbb2", "#ffffff", "\"#ffffff\""],
        "coffee": ["Coffee", "#cfb797","#755849", "\"#755849\""],
        "hehe": ["Hehe", "#626264", "#1D2A48", "\"#1D2A48\""],
        "izumi": ["Izumi", "#ffbdcb", "#e3d5f6", "\"#e3d5f6\""],
        "joy": ["Joy", "#ec556a", "#000000", "\"#000000\""],
        "lightpurple": ["Light Purple", "#D1ADDA", "#F3CFE9", "\"#F3CFE9\""],
        "sakura": ["Sakura", "#FFEBEE", "#EF9A9A", "\"#EF9A9A\""],
        "tea": ["Tea", "#eff0df", "#a6ab5f", "\"#a6ab5f\""],
        "winnie": ["Winnie", "#fae769", "#6d4016", "\"#6d4016\""],
    }

    colors.keys = Object.keys(colors);

    // generating color selection bar

    let setColors = function() {
        let colorDiv = $("#colors");
        for (let i = 0; i < colors.keys.length; i++) {
            let c = colors.keys[i];
            let colorCard = $("<div class='colorCard'>");
            let color1 = $("<div class='color1'>");
            let color2 = $("<div class='color2'>");
            let colorTag = $("<div class='colorTag'>");
            color1.css("background-color", colors[c][1]);
            color2.css("background-color", colors[c][2]);
            colorTag.html(colors[c][0]);
            colorCard.prop("id", c)
            colorCard.append(color1);
            colorCard.append(color2);
            colorCard.append(colorTag);
            colorDiv.append(colorCard);
        }
    };
    ...
    var showColorsForImage = function($image, $imageSection) {
        ...
        setTimeout(function() {
            ...
            if ((currentScrollPosition < outputOffsetTop) && (currentScrollPosition + windowHeight - 250 < outputOffsetTop)) {
                $('body').animate({
                    scrollTop: outputOffsetTop - windowHeight + 200 + "px"
                });
            }
        }, 300);
        return color;
            ...
            if (file.type.match(imageType)) {
                ...
                reader.onload = function(event) {
                    ...
                    $image.on('load', function() {
                        col = showColorsForImage($image, $imageSection);
                        loadCanvas($image, col)
                    });
                };
    ...
    $("#uploadButton").on("click", function() {
        console.log("click");
        let input = $("<input id='pic' type='file' name='img' />");
